...
function headerTapeFunc() {
  ...
  function mbHeader() {
    ...
    if ($(window).width() < 1024) {
      $page_wrap.css({ "padding-top": $header_wrap.outerHeight(true) });
    } else {
...
function whereContainerFunc() {
  const sort_cateitem = $(".sort_cateitem");
  const sort_catecont_wrap = $(".sort_catecont_wrap");
  const sort_chkitem_container = $(".sort_chkitem_container");
  sort_cateitem.on("click", function (e) {
    e.preventDefault();
    const $this = $(this);
    const $target = $($this.attr("href"));
    sort_cateitem.removeClass("active");
    $this.addClass("active");
    if ($target) {
      sort_catecont_wrap.removeClass("active");
      $target.addClass("active");
    }
  });
}
...
function posLayerEvent() {
  ...
  $(document).on("click", ".btn_psubmit", function (e) {
    ...
    let thisSubParent = thisParent.find(".sort_chkitem_container");
    let targetCols = $(`[data-poslayer='#${thisParent.attr("id")}']`);
    let activeDate = thisParent.attr("data-date");
    let activeText = thisParent.find(".pclayer_vlist .pcv_chk.active").text();
    let activeSccOneText = thisParent.find(".sort_cateitem_title.active").text();
  ...
  $(document).on("click", ".pcv_chk , .scc_item, .sort_cateitem_title", function (e) {
    e.preventDefault();
...
function posLayerShow(target, btn) {
  ...
  $("body").append(target);
  if (target.hasClass("mb_full") && $(window).width()<1024 && "ontouchstart" in window) {
